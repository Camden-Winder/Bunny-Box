...
2. Read `CLAUDE.md` for project conventions, rules, and repo layout.
...
2. **Check whether a new LESSONS.md entry is warranted** (see criteria below). If yes, add it in the same commit.
...
- The same root cause appeared across two or more sessions before being understood.
...
- **Triggered by:** session date or issue #N
...
Example:
- `Q2/install-bb-q2.sh` — functions named in Tasks 1 and 2
...
| `HANDOFF.md` | Add session entry at top |
